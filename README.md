...
| 갤러리 | 완료 이미지 격자, 프롬프트·시드 검색, 완료/실패 필터, 상세 보기(← → 로 이동), 설정 불러오기, 같은 시드·새 시드로 재생성, 다운로드, 개별·선택 삭제 |
| 취소 | 대기 중 작업은 대기열에서 제거하고, 실행 중 작업은 ComfyUI 인터럽트 또는 mflux 프로세스 종료로 중단합니다 |
| 이미지 편집 | 참조 이미지를 최대 3장 올리거나(끌어다 놓기 지원) 갤러리 이미지를 "이 이미지 편집하기"로 불러온 뒤, 프롬프트에 편집 지시를 쓰면 Qwen-Image-2.1 의 편집 기능으로 생성합니다. 출력 크기는 첫 참조 이미지에 맞추거나 직접 지정할 수 있습니다. mflux 엔진에서는 첫 이미지만 img2img(변경 강도 조절)로 씁니다 |
