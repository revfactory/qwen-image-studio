...
| 갤러리 | 완료 이미지 격자, 프롬프트·시드 검색, 완료/실패 필터, 상세 보기(← → 로 이동), 설정 불러오기, 같은 시드·새 시드로 재생성, 참조 이미지로 추가(선택 모드에서 최대 3장 한 번에), 이 이미지 편집하기, 다운로드, 개별·선택 삭제 |
...
| 배치 편집 | 참조 이미지를 여러 장(최대 100장) 올리거나 보관함·갤러리에서 고른 뒤 "각 이미지에 따로 적용"을 선택하면, 같은 프롬프트를 각 이미지에 적용한 작업이 한 장마다 하나씩 만들어집니다. 4장 이상 넣으면 자동으로 이 모드가 됩니다 |
